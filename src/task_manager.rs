use crate::task::{Task, KanbanCategory};
...
impl TaskManager {
    ...
    pub fn get_tasks_by_category(&self) -> HashMap<KanbanCategory, Vec<&Task>> {
        ...
        for task in &self.tasks {
            ...
            categorized.entry(category).or_insert_with(Vec::new).push(task);
        }
        ...
        self.tasks
            ...
            .filter(|task| {
                task.title.to_lowercase().contains(&query_lower)
                    ...
                    || task.todos.iter().any(|todo| {
                        todo.text.to_lowercase().contains(&query_lower)
                    })
            })
            .collect()
    }
